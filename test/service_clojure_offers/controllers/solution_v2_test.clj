(ns service-clojure-offers.controllers.solution-v2-test
...
(deftest create-offer-v3-test
  (let [customer-id (utils/uuid)
        ...
        _ (c.offers/create-offer-v3! datomic first-offer)
        offers (d.offers/find-by-customer-id! (config/db! conn) customer-id)]
    (is (= (count offers) 1))))

(deftest validate-second-solution-test
  (let [customer-id (utils/uuid)
        conn (config/connect! config/base-uri config/db-name-offers)]
    (utils/run-threads
      100
